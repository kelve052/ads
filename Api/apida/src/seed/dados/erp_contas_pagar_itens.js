...
const IdContaspagar = async () => {
	// buscar todos os id das copras do banco de dados
	const compras = await prisma.erp_compras.findMany({ select: { id: true, }, });

	// buscar todos os id das contas a pagar do banco de dados 
	const contas = await prisma.erp_contas_pagar.findMany({ select: { id: true, }, });

	// pegar todas as conta a pagar que não estão relacionadas a nenhuma compra
	const contasSelecionadas = contas.filter(conta => !compras.some(compra => compra.erp_contas_pagar_id === conta.id));

	return contasSelecionadas.map((conta) => conta.id);
}
console.log(await IdContaspagar());
